...
interface
uses
  SysUtils, Classes, LexerTokenTypeUnit;
...
type
  ...
  TLexerType= (ltStart, ltJava, ltDocument,
              ltIntVector, ltMSEQuery, ltPascal, ltNone, ltEnd);
...
type
  ...
  private
    ...
    function PascalRead (Stream: TStream): TChar;
    
  published
    function GetNextChar (Stream: TStream): TChar;
  ...
  public
    ...
    constructor Create (LexType: TLexerType; LastChar: TChar); overload;
    constructor Create (LexType: TLexerType); overload;
    function Copy: TToken;
    procedure Free;
    
    function ReadFromFile (var FileHandle: TCharFile): TChar; overload;
    function ReadFromStream (Stream: TStream): TChar; overload;
    function ReadFromStream (Stream: TStream; LastChar: TChar): TChar; virtual; abstract; overload;
...
//  Result:= TLexerSavePoint.Create (FilePos (FFileHandle), CurrentToken, FLastChar);
  Result:= TLexerSavePoint.Create (FStream.Position, CurrentToken, FLastChar);
...
//  BlockRead (FFileHandle, Data, Position.FilePointerLocation, Res);
  FStream.Position:= Position.FilePointerLocation;
  FLastChar:= Position.LastChar;
//  WriteLn ('CurToken:'+ FCurrentToken.ToString);
end;
...
begin
  ...
  Result:= TToken.Create (FLexerType, FLastChar);
  FLastChar:= Result.ReadFromStream (FStream);
  ...
  begin
    ...
    Result:= TToken.Create (FLexerType, FLastChar);
    FLastChar:= Result.ReadFromStream (FStream);
//    FLastChar:= Result.ReadFromFile (FFileHandle);
...
function TToken.GetNextChar (Stream: TStream): TChar;
var
  CharKind: TCharKind;
  Ch: Char;
...
constructor TToken.Create (LexType: TLexerType; LastChar: TChar);
begin
  inherited Create;
...
constructor TToken.Create (LexType: TLexerType);
begin
  inherited Create;
...
function TToken.PascalRead (Stream: TStream): TChar;
var
  Ch, TempCh: TChar;
  KeyWordIter: TKeywordType;
...
function TToken.ReadFromFile (var FileHandle: TCharFile): TChar;
begin
  case LexerType of
    ltJava:
      Result:= JavaReadFromFile (FileHandle);
    ltDocument:
      Result:= DocumentReadFromFile (FileHandle);
    ltIntVector:
      Result:= DocumentReadFromFile (FileHandle)
    else
      raise EInvalidLexerConfiguration.Create ('Invalid LexerType');
      
  end;
  
end;

function TToken.ReadFromStream (Stream: TStream): TChar;
begin

  case LexerType of
    ltJava:
      Result:= JavaRead (Stream);
    ltDocument:
      Result:= DocumentRead (Stream);
    ltIntVector:
      Result:= DocumentRead (Stream);
    ltPascal:
      Result:= PascalRead (Stream);
    else
      raise EInvalidLexerConfiguration.Create ('Invalid LexerType');
