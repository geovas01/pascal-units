...
procedure TVotingEntryCollection.Free;
var
  i: Integer;
...
procedure TVotingEntryCollection.LoadBitmap (Bitmap: TBitmap);
  ...
  begin
    ...
    if (ActiveComponent.MinX= 0) or (BotRightSearchPoint.x-
      TopLeftSearchPoint.x<= ActiveComponent.MaxX) or
      (ActiveComponent.MinY= 0) or (BotRightSearchPoint.y-
      TopLeftSearchPoint.y<= ActiveComponent.MaxY) then
    else
      Exit;

    if (ActiveComponent.Width< MinImagePropertiesForSpecialCase.x) or
       (ActiveComponent.Height< MinImagePropertiesForSpecialCase.y) then
    begin
      Result:= True;
      Exit;
    ...
    for i:= 0 to ComponentCollection.Count- 1 do
      ...
      begin
        ...
        PointAvg:= Trunc (ComponentCollection.Component [i].Count/
            (ComponentCollection.Component [i].MaxX-
            ComponentCollection.Component [i].MinX));
