...
begin
  ...
  RStr:= System.Copy (Str, 0, Pos (':', Str)- 1);
...
begin
  Result:= Self;
  Result.r:= Round (Result.Fr* Cos (Angle)- Result.Fc* Sin (Angle));
  Result.c:= Round (Result.Fc* Sin (Angle)+ Result.Fc* Cos (Angle));
...
procedure TPointCollection.Free (FreeObj: Boolean);
var
  i: Integer;
  Ptr: ^TObject;
...
procedure TBoxCollection.Free (FreeObj: Boolean);
var
  i: Integer;
  Ptr: PObject;
