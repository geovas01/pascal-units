...
begin
  for i:=FParamList.Count-1 downto 0 do
    Dispose(PHTMLParameter(FParamList[I]));
  FParamList.Free;
  inherited Destroy;
end;
...
procedure THtmlInfo.ParseTag;
  ...
  begin
    ...
    if Position<>0 then
    if FText[Position] = '=' then  // This parameter has a value (some parameter's don't!)
   ...
   if CompareStr(Copy(FTagName, 1, 3), '!--') = 0 then   // This is a comment - has no parameters
    begin
      ...
      NewParam := CreateNewParam;
        ...
        if ParamStr <> '' then
          while (Length(ParamStr) <> 0) and ((ParamStr[Length(ParamStr)] = ' ') or (ParamStr[Length(ParamStr)] = '-')) do
...
//      Application.ProcessMessages;
  ...
  for i:=0 to Self.Count-1 do
    if Self.Item[i].IsTag then
      if UCASE(Self.Item[i].TagName)='META' then
      begin
        Temp:=UCase(Self.Item[i].FText);
        if Pos('CHARSET',Temp)>0 then
        begin
          Self.CharSet:=Copy(Temp,
           Pos('CHARSET',Temp)+8,Length(Temp));
          Temp:=Self.CharSet;
          Delete(Temp,Length(Temp),1);
          Self.CharSet:=UCase(Temp);
          if Self.CharSet='UTF-8' then
          begin
            IsUnicode:=True;
            Exit
          end
