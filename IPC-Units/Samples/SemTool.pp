...
begin
  With lock do
   ...
   WriteLn ('Me,ber= ', member);
  WriteLn ('GetMemberCount (ID)= ', GetMemberCount (ID));
...
begin
  ...
  Case UpCase(S [1][1]) of
   'C' : begin
//         if paramcount<>2 then usage;
         WriteLn (CreateSem (key,strtoint(S [2])));
         end;
   'L' : begin
//         if paramcount<>2 then usage;
         ID:=OpenSem (key);
         WriteLn (ID);
         LockSem (ID,strtoint(S [2]));
         end;
   'U' : begin
//         if paramcount<>2 then usage;
         ID:=OpenSem (key);
         UnLockSem (ID,strtoint(S [2]));
         end;
   'M' : begin
//         if paramcount<>2 then usage;
         ID:=OpenSem (key);
         ChangeMode (ID,strtoint(S [2]));
         end;
   'D' : Begin
         ID:=OpenSem(Key);
         RemoveSem(Id);
