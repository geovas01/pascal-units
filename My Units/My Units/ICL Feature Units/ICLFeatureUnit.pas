...
const
  ...
  CONST_USE_REGION_LABELS_FOR_DEBUG = True;
...
type
  ...
  public
    property BaseLineHeightCoef: Integer read FBaseLineHeightCoef;
    property BaseLineWidthCoef: Integer read FBaseLineWidthCoef;
    property BaseLineHeight: Integer read FBaseLineHeight;
...
uses Math, ExceptionUnit, VarUtils;
type
...
begin
  ...
  if FComponents.Size= 0 then
    raise EInvalidComponent.Create ('BlackImage');
...
begin
  ...
  for i:= 0 to FComponents.Size- 1 do
    FComponents.Component [i].FPercentageOfPointsInComp:=
      (FComponents.Component [i].FPointCollection.Size* 100) div  (FRows* FColumns){TotalWhitePixCount};

  {
  for i:= FComponents.Size- 1 downto 0 do
  begin
    TICLFeatureComponent (CompPtr^).FPercentageOfPointsInComp:=
      (TICLFeatureComponent (CompPtr^).FPointCollection.Size* 100) div  (FRows* FColumns);
    Dec (CompPtr);
  end; }
  ...
  begin
    if (100* TICLFeatureComponent (CompPtr^).FPointCollection.Size <
       ...
       and (TICLFeatureComponent (CompPtr^).FICLLabel <> Ord (iclHole)) then
    begin
      TICLFeatureComponent (CompPtr^).Free;
...
procedure TFreemanFeature.Free (FreeObj: Boolean= True);
var
  i: Integer;
  Ptr: ^TObject;
...
begin
  ...
  begin
    ...
    begin
      TARowOfFreemanFeature (Ptr^).Free;
      Inc (Ptr);
...
procedure TICLFeatureComponent.Free ;
begin

  HashedData.Free;
  FCenterOfMass.Free;
  FPointCollection.Free (True);
...
procedure TICLFeatureComponentCollection.Free (FreeObj: Boolean= True);
var
  i: Integer;
...
begin
  ...
  else if AnotherICLDescriptor.FLabel= Ord (iclHole) then
  begin
...
begin
  WriteLn (OutputFile, FCompAreaRegardingImArea);
  WriteLn (OutputFile, FCOMCordRegardingImage.ToString);
  WriteLn (OutputFile, FLabel);
  WriteLn (OutputFile, FPointsInRegion);
  WriteLn (OutputFile, FComponentHeightRegImHeight);
  WriteLn (OutputFile, FComponentWidthRegImWidth);

end;

function TICLDescriptor.ToString: String;
begin
  Result:= IntToStr (FLabel)+ ': RelSize= '+ IntToStr (FCompAreaRegardingImArea)+ #13+
...
procedure TICLNeighborRegion.Free;
begin
...
procedure TICLNeighborRegionCollection.Free (FreeObj: Boolean= True);
var
  Ptr: PObject;
  i: Integer;
...
begin
  ...
  begin
    ...
    if TICLDescriptor (Ptr1^).FLabel= Ord (iclHole) then
      w:= 2* w;
  ...
  begin
    ...
    if TICLDescriptor (Ptr2^).FLabel= Ord (iclHole) then
      w:= 2* w;
...
(*$endif*)
  ...
  begin
    ...
    begin
      ...
      if AnotherICLDescCol.ICLDescriptor [j].FLabel= Ord (iclHole) then
        w:= 2* w;
...
procedure TICLDescriptorCollection.Free (FreeObj: Boolean);
var
  i: Integer;
  Ptr: ^TObject;
...
begin
  WriteLn (OutputFile, FImageWidth);
  WriteLn (OutputFile, FImageHeight);
  WriteLn (OutputFile, FBlackPercOnBaseLine);
  WriteLn (OutputFile, FCOMRelPos.ToString);
...
procedure TICLClassDefinition.Free (FreeObj: Boolean);
var
  i: Integer;
  Ptr: PObject;
...
begin
  ...
  for i:= 1 to Size do
  begin
    TICLDescriptorCollection (Ptr^).Free;
    Inc (Ptr);
...
procedure TICLClassDefinitionCollection.Free (FreeObj: Boolean);
var
  i: Integer;
  Ptr: PObject;
