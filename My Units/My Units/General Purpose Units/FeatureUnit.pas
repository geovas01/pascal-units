...
type
  ...
  private
    ...
    function GausianFilter (Center: TPoint; DistanceBetweenSamples: Extended; ThirdIndex: Integer): Extended;
...
procedure TMultiDimensionalArray.Free;
begin
  SetLength (FSizes, 0);
...
begin
  ...
  ActiveImage:= GradiantFeature [ThirdIndex];

  r1:= Math.Max (-HalfOfMaskSize, -Center.r)+ Center.r ;
  for r:= Math.Max (-HalfOfMaskSize, -Center.r) to
          Min (HalfOfMaskSize, FRow- Center.r- 1) do
  begin
    c1:= Math.Max (-HalfOfMaskSize, -Center.c)+ Center.c;

    Ptr:= ActiveImage.ScanLine [r1];
    Inc (Ptr, c1);
...
procedure T8DirGradiantFeature.Free;
var
  i: Integer;

begin
  for i:= 0 to Size- 1 do
    GradiantFeature [i].Free;
...
var
  ...
  i, j, k: Integer;
  DistanceBetweenSamples: Extended;

begin
  Result:= TFeatureVectorBasedOnGradiant.Create;
  DistanceBetweenSamples:= FRow/ NumberOfMasks;
  ...
  begin
    ...
    for j:= 0 to NumberOfMasks- 1 do
      Centers [i, j]:= TPoint.Create (
        Round ( (i+ 0.5)* DistanceBetweenSamples),
        Round ( (j+ 0.5)* DistanceBetweenSamples));
...
procedure TTwoDimensionDoubleArray.Free;
var
  r: Integer;
...
procedure TFeatureVectorBasedOnGradiantCollection.Free (FreeObj: Boolean);
var
  i: Integer;
...
begin
  if FreeObj then
    for i:= 0 to Size- 1 do
      FeatureVectorBasedOnGradiant [i].Free;
